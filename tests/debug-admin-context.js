  ...
  try {
    ...
    const pageDebug = await page.evaluate(() => {
      return {
        ...
        reactChildren: document.getElementById('root')
    ...
    const campaignsDebug = await page.evaluate(() => {
      return {
        ...
        bodyLength: document.body.innerText.length,
        hasContent:
          document.body.innerText.includes('Campaign') ||
          document.body.innerText.includes('Management'),
      };
    });
    ...
    console.log('=========================================');
    console.log(
      `Admin Login: ${adminState.currentUrl.includes('dashboard') ? 'Success' : 'Failed'}`
    );
    console.log(
      `Users Page: ${pageDebug.bodyLength > 100 ? 'Has Content' : 'Empty'} (${pageDebug.bodyLength} chars)`
    );
    console.log(
      `Campaigns Page: ${campaignsDebug.bodyLength > 100 ? 'Has Content' : 'Empty'} (${campaignsDebug.bodyLength} chars)`
    );
