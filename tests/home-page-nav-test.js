...
async function testHomePageNavigation() {
  ...
  try {
    ...
    const heroButtons = await page.$$(
      'button, a[href*="campaign"], button:has-text("Get Started"), button:has-text("Start"), button:has-text("Campaign")'
    );
    ...
    if (heroButtons.length > 0) {
      ...
      for (const button of heroButtons) {
        ...
        if (
          buttonText.toLowerCase().includes('campaign') ||
    ...
    const ctaButtons = await page.$$(
      'button:has-text("Get Started"), button:has-text("Start"), a[href*="campaign"]'
    );
