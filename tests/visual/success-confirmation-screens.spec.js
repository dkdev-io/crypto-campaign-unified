...
test.describe('Success Confirmation Screens Visual Tests', () => {
  ...
  test('campaign setup completion success', async ({ page }) => {
    ...
    const setupButton = page
      .locator('button, a')
      .filter({ hasText: /setup|create|campaign/i })
      .first();
    ...
    if (await titleInput.isVisible()) {
      ...
      const submitButton = page
        ...
        .filter({ hasText: /create|launch|finish/i })
        .first();
      if (await submitButton.isVisible()) {
        await submitButton.click();
        await page.waitForTimeout(3000);
  ...
  test('newsletter signup confirmation', async ({ page }) => {
    ...
    const newsletterInput = page
      .locator('input[name*="newsletter"], input[name*="subscribe"]')
      .first();

    const targetInput = (await emailInput.isVisible())
      ? emailInput
    ...
    if (targetInput) {
      ...
      const subscribeButton = page
        .locator('button')
        .filter({ hasText: /subscribe|sign up/i })
        .first();
    ...
    if (await amountInput.isVisible()) {
      ...
      if (await submitButton.isVisible()) {
        ...
        const shareButtons = page
          .locator('button, a')
          .filter({ hasText: /share|twitter|facebook|linkedin/i });
  ...
  test('success confirmation with receipt download', async ({ page }) => {
    ...
    if (await submitButton.isVisible()) {
      ...
      const downloadButton = page
        .locator('button, a')
        .filter({ hasText: /download.*receipt|receipt.*download/i })
        .first();
  ...
  test('recurring contribution setup success', async ({ page }) => {
    ...
    const recurringCheckbox = page
      ...
      .filter({ hasText: /recurring|monthly|weekly/i })
      .first();
  ...
  test('team member success confirmation', async ({ page }) => {
    ...
    const adminButton = page
      .locator('button, a')
      .filter({ hasText: /admin|team|manage/i })
      .first();
