...
async function verifyAdminDashboard() {
  ...
  try {
    ...
    const campaignCount = await page.$eval('.crypto-card .text-3xl', el => el.textContent.trim()).catch(() => '0');
    ...
    console.log(`✅ Campaign Data: ${campaignSuccess ? '✅ CORRECT' : '❌ WRONG'} (${campaignCount})`);
    console.log(`✅ Revenue Data: ${revenueSuccess ? '✅ CORRECT' : '❌ WRONG'} (${revenueValue})`);
    console.log(`✅ Transactions: ${hasTransactions ? '✅ SHOWING' : '❌ EMPTY'} (${transactionRows.length} rows)`);
...
// Run verification
verifyAdminDashboard().then(result => {
