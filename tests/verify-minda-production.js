...
async function verifyMindaProduction() {
  ...
  try {
    ...
    if (hasAccessDenied || hasUserManagement) {
      console.log(
        `   ✅ Users page shows content: ${hasAccessDenied ? 'Access Denied' : 'User Management'}`
      );
      if (hasAdminRole) {
    ...
    const hasCampaignContent =
      campaignsPageContent.includes('Campaign') || campaignsPageContent.length > 100;
    ...
    console.log(
      `Users Page: ${hasAccessDenied || hasUserManagement ? '✅' : '❌'} ${hasAccessDenied ? 'Access Denied' : hasUserManagement ? 'Working' : 'Still Blank'}`
    );
    console.log(
      `Campaigns Page: ${hasCampaignContent ? '✅' : '❌'} ${hasCampaignContent ? 'Working' : 'Blank'}`
    );
    console.log(`Console Errors: ${logs.length === 0 ? '✅' : '❌'} ${logs.length} errors`);

    const success =
      dashboardCards.length > 0 && (hasAccessDenied || hasUserManagement) && hasCampaignContent;
    ...
    return {
      ...
      details: {
        ...
        usersWorking: hasAccessDenied || hasUserManagement,
        campaignsWorking: hasCampaignContent,
        errors: logs.length,
      },
    };
  } catch (error) {
