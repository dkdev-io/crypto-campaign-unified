...
describe('Full Dataset Validation (515 Records)', function () {
  ...
  describe('1. Data Integrity Validation', function () {
    ...
    it('should have 150 prospect records with all required fields', function () {
      ...
      prospects.forEach((prospect, index) => {
        ...
        expect(
          ...
          `Prospect ${index} has invalid wallet address: ${prospect.wallet_address}`
        ).to.be.true;
      });
    });
    ...
    it('should have 215 donor records with valid amounts', function () {
      ...
      donors.forEach((donor, index) => {
        ...
        expect(
          amount,
          `Donor ${index} exceeds max contribution: ${donor.amount}`
        ).to.be.lessThanOrEqual(3300);
      });
    });
    ...
    it('should have 150 KYC records with valid statuses', function () {
      ...
      kycRecords.forEach((kyc, index) => {
        ...
        expect(
          ethers.isAddress(kyc.wallet_address),
          `KYC ${index} has invalid wallet address: ${kyc.wallet_address}`
        ).to.be.true;
        expect(validStatuses.includes(kyc.status), `KYC ${index} has invalid status: ${kyc.status}`)
          .to.be.true;
      });
    });
  });
  ...
  describe('3. Contribution Simulation (Sample Donors)', function () {
    ...
    before(function () {
      ...
      verifiedDonors = donors
        .filter(
          (donor) =>
            donor.wallet_address && approvedAddresses.includes(donor.wallet_address.toLowerCase())
        )
  ...
  describe('4. Data Relationship Validation', function () {
    it('should validate prospect-donor relationships', function () {
      const prospectAddresses = prospects
    ...
    it('should validate KYC-donor relationships', function () {
      ...
      const donorsWithApprovedKYC = donorAddresses.filter((addr) =>
        kycApprovedAddresses.includes(addr)
      );
  ...
  describe('5. Performance and Scale Testing', function () {
    ...
    it('should maintain data consistency under load', async function () {
      ...
      results.forEach((stats) => {
        ...
        expect(stats.uniqueContributors.toString()).to.equal(
          initialStats.uniqueContributors.toString()
        );
      });
  ...
  describe('6. Edge Cases and Error Handling', function () {
    ...
    it('should reject contributions from unverified addresses', async function () {
      const unverifiedRecord = kycRecords.find(
      ...
      await expect(contract.connect(account).contribute({ value: amount })).to.be.revertedWith(
        'CampaignContributions: KYC verification required'
      );
    });
  ...
  describe('7. MVP Readiness Checklist (Full Dataset)', function () {
    ...
    it('✓ Data relationships and integrity maintained', function () {
      const totalValidAddresses = [...prospects, ...donors, ...kycRecords].filter(
        (record) => record.wallet_address && ethers.isAddress(record.wallet_address)
      ).length;
