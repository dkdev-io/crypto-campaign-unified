...
async function verifyCommitteeFormUI() {
  ...
  try {
    ...
    await page.waitForSelector('input[placeholder="Enter committee name or keywords..."]', {
      timeout: 10000,
    });
    ...
    try {
      ...
      await Promise.race([
        ...
        page.waitForFunction(() => window.location.pathname === '/BankConnection', {
          timeout: 10000,
        }),
        page.waitForSelector('div:has-text("Committee information saved")', { timeout: 10000 }),
      ]);
      ...
      if (
        currentUrl.includes('/BankConnection') ||
        (await page.$('div:has-text("Committee Information Saved!")'))
      ) {
