...
async function verifyFormStyling() {
  ...
  try {
    ...
    const stylingInfo = await page.evaluate(() => {
      ...
      return {
        setupCardBackground: setupCard
          ? window.getComputedStyle(setupCard).backgroundColor
          : 'not found',
        inputCount: formInputs.length,
        firstInputBackground: formInputs[0]
          ? window.getComputedStyle(formInputs[0]).backgroundColor
          : 'no inputs',
        firstInputColor: formInputs[0] ? window.getComputedStyle(formInputs[0]).color : 'no inputs',
        stepIndicatorBackground: stepIndicator
          ? window.getComputedStyle(stepIndicator).backgroundColor
          : 'not found',
        stepIndicatorColor: stepIndicator
          ? window.getComputedStyle(stepIndicator).color
          : 'not found',
      };
    });
