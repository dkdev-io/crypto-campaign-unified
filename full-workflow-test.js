...
async function fullWorkflowTest() {
  ...
  try {
    ...
    results.summary.overallPercentage = Math.round(
      (results.summary.emojisRemoved + results.summary.backgroundsFixed + results.summary.fontsFixed) / 3
    );
    ...
    validSteps.forEach(step => {
      ...
      console.log(`  Emojis: ${step.emojisFound === 0 ? '✅ None' : '❌ Found ' + step.emojisFound}`);
...
async function testCurrentStep(page, homeRef, stepNumber) {
  try {
    return await page.evaluate((homeRef, stepNumber) => {
      const body = document.body;
      const bodyStyles = window.getComputedStyle(body);
      ...
      return {
        ...
        backgroundMatches: (containerStyles?.backgroundColor || bodyStyles.backgroundColor) === homeRef.backgroundColor,
        ...
        headingUsesDesignSystem: mainHeading?.getAttribute('style')?.includes('var(--text-heading') || false,
