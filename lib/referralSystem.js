...
export class ReferralSystem {
   ...
   * Uses fallback client-side generation if database function fails
  ...
  async generateReferralCode(donorName) {
    try {
      ...
      console.warn('Database function not available, using fallback referral code generation');
      return await this._generateReferralCodeFallback(donorName);
      
    } catch (error) {
      console.warn('Database function failed, using fallback referral code generation');
      return await this._generateReferralCodeFallback(donorName);
    }
  }

  /**
   * Fallback method to generate unique referral codes client-side
   * @private
   */
  async _generateReferralCodeFallback(donorName) {
    // Create base code from donor name (first 3 chars + 4 random chars)
    const cleanName = donorName.replace(/[^A-Za-z]/g, '');
    let baseCode = cleanName.substring(0, 3).toUpperCase();
  ...
  async createOrGetDonor({ email, name, walletAddress, phone }) {
    try {
      ...
      // Create new donor record (using existing schema)
      const donorRecord = {
        email,
        full_name: name,
        phone: phone || null,
  ...
  async getCandidates() {
    try {
      ...
      // Transform campaigns to candidate format (using actual schema)
      return (campaigns || []).map(campaign => ({
        id: campaign.id,
        name: campaign.campaign_name || campaign.title,
        description: campaign.description || `Campaign for ${campaign.campaign_name || campaign.title}`,
        wallet_address: campaign.wallet_address,
        campaign_goal: campaign.max_donation_limit || '3300',
        total_raised: '0', // Start at 0
