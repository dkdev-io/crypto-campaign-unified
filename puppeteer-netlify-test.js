...
async function testNetlifySignup() {
  ...
  try {
    ...
    const hasAuth = pageContent.toLowerCase().includes('auth') ||
    ...
    try {
      ...
      const emailInput = await page.$('input[type="email"], input[name="email"], #email, #signup-email');
      if (emailInput) {
        ...
        if (signupButtons.length > 0) {
          ...
          await page.evaluate(() => {
            const buttons = Array.from(document.querySelectorAll('button'));
            const signupBtn = buttons.find(btn => 
              btn.textContent.toLowerCase().includes('sign up') ||
              btn.textContent.toLowerCase().includes('signup') ||
              btn.textContent.toLowerCase().includes('register')
            );
        ...
        const nameInput = await page.$('input[name="fullName"], input[name="name"], #fullName, #name');
        if (nameInput) {
        ...
        if (submitButton) {
          ...
          if (finalContent.toLowerCase().includes('verification') || 
