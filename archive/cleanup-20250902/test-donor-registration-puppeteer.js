...
async function testDonorRegistrationWithPuppeteer() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        '   Make sure the Netlify site is accessible at https://cryptocampaign.netlify.app'
      );
      return false;
    }
    ...
    const nameInput = await page.$(
      'input[name="fullName"], input[name="full_name"], #fullName, #full_name'
    );
      ...
      await page.waitForFunction(
        () => {
          ...
          return (
            ...
            !window.location.pathname.includes('/donors/auth/register')
    ...
    try {
      ...
      if (userError) {
        ...
        if (createdUser) {
          ...
          console.log(
            `✅ Email confirmed: ${createdUser.email_confirmed_at ? 'Yes' : 'No (needs verification)'}`
          );
        } else {
...
// Run the test
testDonorRegistrationWithPuppeteer()
  .then((result) => {
    console.log('\n' + '='.repeat(60));
    if (result && result.success) {
      ...
      console.log(
        `   • Network responses: ${result.networkResponse ? 'Received' : 'None detected'}`
      );
