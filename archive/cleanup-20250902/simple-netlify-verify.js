...
async function verifyChanges() {
  ...
  try {
    ...
    const homeNav = await page.evaluate(() => {
      ...
      // Find NEXTRAISE logo
      results.logo = Array.from(document.querySelectorAll('*')).find(
      ...
      results.features = Array.from(document.querySelectorAll('*')).find(
        (el) => el.textContent && el.textContent.includes('FEATURES')
      )
        ? 'Found'
        : 'Missing';

      results.howItWorks = Array.from(document.querySelectorAll('*')).find(
        (el) => el.textContent && el.textContent.includes('HOW IT WORKS')
      )
        ? 'Found'
        : 'Missing';

      results.pricing = Array.from(document.querySelectorAll('*')).find(
        (el) => el.textContent && el.textContent.includes('PRICING')
      )
        ? 'Found'
        : 'Missing';

      results.contact = Array.from(document.querySelectorAll('*')).find(
        (el) => el.textContent && el.textContent.includes('CONTACT')
      )
        ? 'Found'
        : 'Missing';

      // Find action buttons
      results.campaignsBtn = Array.from(document.querySelectorAll('button, .btn')).find(
        (el) => el.textContent && el.textContent.includes('Campaigns')
      )
        ? 'Found'
        : 'Missing';

      results.donorsBtn = Array.from(document.querySelectorAll('button, .btn')).find(
    ...
    const authElements = await page.evaluate(() => {
      ...
      results.signInTab = Array.from(document.querySelectorAll('button, [role="tab"]')).find(
        (el) => el.textContent && el.textContent.trim() === 'Sign In'
      )
        ? 'Found'
        : 'Missing';

      results.signUpTab = Array.from(document.querySelectorAll('button, [role="tab"]')).find(
      ...
      results.homeLink = Array.from(document.querySelectorAll('a, span')).find(
        (el) => el.textContent && el.textContent.includes('Home')
      )
        ? 'Found'
        : 'Missing';

      results.campaignsLink = Array.from(document.querySelectorAll('a, span')).find(
        (el) => el.textContent && el.textContent.includes('Campaigns')
      )
        ? 'Found'
        : 'Missing';
      ...
      // Check form elements
      results.emailField = document.querySelector('input[type="email"], input[name="email"]')
        ? 'Found'
        : 'Missing';
      results.passwordField = document.querySelector('input[type="password"]')
        ? 'Found'
        : 'Missing';
    ...
    try {
      ...
      if (signUpButton) {
        ...
        const signUpFormElements = await page.evaluate(() => {
          return {
            fullNameField: document.querySelector(
    ...
    const campaignsAuthTest = await page.evaluate(() => {
      ...
      return {
        ...
        hasToggle: Array.from(document.querySelectorAll('button')).some(
          (el) =>
            el.textContent &&
    ...
    console.log(
      `🔄 Toggle Functionality: ${campaignsAuthTest.hasToggle ? 'Working' : 'Issues detected'}`
    );

    const overallStatus =
      homeNavScore >= 5 && authElementsScore >= 6 && campaignsAuthTest.hasToggle
