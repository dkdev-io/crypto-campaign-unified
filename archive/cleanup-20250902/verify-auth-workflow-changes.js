...
async function verifyAuthWorkflow() {
  ...
  try {
    ...
    const homeNavigation = await page.evaluate(() => {
      const logo =
        document.querySelector('span:contains("NEXTRAISE")') ||
      ...
      const howItWorksLink = document.querySelector(
        'a[href="#how-it-works"], [href*="how-it-works"]'
      );
      ...
      const campaignsBtn =
        document.querySelector('button:contains("Campaigns")') ||
        document.querySelector('[class*="btn"]:contains("Campaigns")');
      const donorsBtn =
        document.querySelector('button:contains("Donors")') ||
    ...
    results.homePageNavigation =
      homeNavigation.hasLogo && homeNavigation.hasCampaignsBtn && homeNavigation.hasDonorsBtn;
    ...
    const authNavigation = await page.evaluate(() => {
      ...
      const logo = Array.from(document.querySelectorAll('*')).find(
        (el) => el.textContent && el.textContent.includes('NEXTRAISE')
      );

      // Check for navigation links
      const featuresLink = Array.from(document.querySelectorAll('a, nav *')).find(
        (el) => el.textContent && el.textContent.toUpperCase().includes('FEATURES')
      );
      const howItWorksLink = Array.from(document.querySelectorAll('a, nav *')).find(
        (el) => el.textContent && el.textContent.toUpperCase().includes('HOW IT WORKS')
      );
      const pricingLink = Array.from(document.querySelectorAll('a, nav *')).find(
        (el) => el.textContent && el.textContent.toUpperCase().includes('PRICING')
      );
      const contactLink = Array.from(document.querySelectorAll('a, nav *')).find(
        (el) => el.textContent && el.textContent.toUpperCase().includes('CONTACT')
      );

      // Check for action buttons
      const campaignsBtn = Array.from(document.querySelectorAll('button, [class*="btn"]')).find(
        (el) => el.textContent && el.textContent.includes('Campaigns')
      );
      const donorsBtn = Array.from(document.querySelectorAll('button, [class*="btn"]')).find(
        (el) => el.textContent && el.textContent.includes('Donors')
      );
    ...
    results.authPageNavigation =
      authNavigation.hasLogo && authNavigation.hasCampaignsBtn && authNavigation.hasDonorsBtn;
    ...
    const toggleTest = await page.evaluate(() => {
      ...
      const signInTab = Array.from(document.querySelectorAll('button, [role="tab"], .tab')).find(
        (el) => el.textContent && el.textContent.trim() === 'Sign In'
      );
      const signUpTab = Array.from(document.querySelectorAll('button, [role="tab"], .tab')).find(
        (el) => el.textContent && el.textContent.trim() === 'Sign Up'
      );

      // Check for proper toggle styling
      const hasToggleContainer =
        document.querySelector('.flex') && document.querySelector('[class*="rounded"]');
    ...
    console.log(
      `   ✅ Sign In Tab: ${toggleTest.hasSignInTab ? '✓' : '✗'} (${toggleTest.signInText})`
    );
    console.log(
      `   ✅ Sign Up Tab: ${toggleTest.hasSignUpTab ? '✓' : '✗'} (${toggleTest.signUpText})`
    );
    ...
    results.signInSignUpToggle =
      toggleTest.hasSignInTab && toggleTest.hasSignUpTab && toggleTest.hasToggleContainer;
      ...
      try {
        ...
        const signUpFormCheck = await page.evaluate(() => {
          const fullNameField = document.querySelector(
            'input[name="fullName"], input[placeholder*="full name" i]'
          );
        ...
        console.log(
          `   ✅ Confirm Password: ${signUpFormCheck.hasConfirmPasswordField ? '✓' : '✗'}`
        );
      } catch (error) {
    ...
    const breadcrumbTest = await page.evaluate(() => {
      ...
      const breadcrumbContainer = document.querySelector(
        '[aria-label="Breadcrumb"], nav[class*="breadcrumb"], .breadcrumb'
      );
      const homeLink = Array.from(document.querySelectorAll('a, [role="link"]')).find(
        (el) => el.textContent && el.textContent.includes('Home')
      );
      const campaignsLink = Array.from(document.querySelectorAll('a, [role="link"], span')).find(
        (el) => el.textContent && el.textContent.includes('Campaigns')
      );
      const authLink = Array.from(document.querySelectorAll('a, [role="link"], span')).find(
        (el) =>
          el.textContent &&
          (el.textContent.includes('Sign In') || el.textContent.includes('Sign Up'))
      );

      // Look for chevron or separator elements
      const chevronElements = document.querySelectorAll(
    ...
    console.log(
      `   ✅ Home Link: ${breadcrumbTest.hasHomeLink ? '✓' : '✗'} (${breadcrumbTest.homeText})`
    );
    console.log(
      `   ✅ Campaigns Link: ${breadcrumbTest.hasCampaignsLink ? '✓' : '✗'} (${breadcrumbTest.campaignsText})`
    );
    console.log(
      `   ✅ Auth Link: ${breadcrumbTest.hasAuthLink ? '✓' : '✗'} (${breadcrumbTest.authText})`
    );
    ...
    const stylingTest = await page.evaluate(() => {
      ...
      const cardElements = document.querySelectorAll(
        '[class*="card"], [class*="rounded"], [class*="shadow"]'
      );
    ...
    console.log(
      `   ✅ Card Styling: ${stylingTest.hasCardStyling ? '✓' : '✗'} (${stylingTest.cardCount} cards)`
    );
    console.log(`   ✅ Gradient Background: ${stylingTest.hasGradientBackground ? '✓' : '✗'}`);
    console.log(
      `   ✅ Proper Headers: ${stylingTest.hasProperHeaders ? '✓' : '✗'} (${stylingTest.headerCount} headers)`
    );
    console.log(
      `   ✅ Styled Buttons: ${stylingTest.hasStyledButtons ? '✓' : '✗'} (${stylingTest.buttonCount} buttons)`
    );
    console.log(`   ✅ Consistent Colors: ${stylingTest.hasConsistentColors ? '✓' : '✗'}`);

    results.overallStyling =
      stylingTest.hasCardStyling && stylingTest.hasProperHeaders && stylingTest.hasStyledButtons;
    ...
    try {
      await page.goto(`${PRODUCTION_URL}/campaigns/auth`, {
      ...
      const campaignsAuthTest = await page.evaluate(() => {
        const pageTitle =
          document.querySelector('h1, h2, h3, [class*="text-3xl"]')?.textContent || '';
        const hasSignInTab = Array.from(document.querySelectorAll('button, [role="tab"]')).some(
          (el) => el.textContent && el.textContent.trim() === 'Sign In'
        );
        const hasSignUpTab = Array.from(document.querySelectorAll('button, [role="tab"]')).some(
          (el) => el.textContent && el.textContent.trim() === 'Sign Up'
        );
      ...
      console.log(
        `   ✅ Has Toggle: ${campaignsAuthTest.hasSignInTab && campaignsAuthTest.hasSignUpTab ? '✓' : '✗'}`
      );
    } catch (error) {
    ...
    console.log(
      `🍞 Breadcrumb Navigation: ${results.breadcrumbNavigation ? '✅ PASS' : '❌ FAIL'}`
    );
    ...
    if (overallScore >= 3) {
      console.log(
        '🎉 The campaign auth workflow changes have been successfully deployed to Netlify!'
      );
