...
async function forceFixCampaigns() {
  ...
  try {
    ...
    const clicked = await page.evaluate(() => {
      ...
      const brandButton = document.querySelector('button.bg-brand, button.bg-green-500, button.bg-blue-500, button[type="submit"]');
      if (brandButton) {
        brandButton.click();
        return true;
      }
    ...
    if (clicked) {
      ...
      const result = await page.evaluate(() => {
        const successElement = document.querySelector('.text-green-500, .bg-green-100, [data-state="success"]');
        const errorElement = document.querySelector('.text-red-500, .bg-red-100, [data-state="error"]');
