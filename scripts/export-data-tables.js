...
class DataExporter {
  ...
  createJSONSummary(validationData) {
    const summary = {
      ...
      statistics: {
        ...
        rejected_single_limit: validationData.filter((r) =>
          r.contract_decision.includes('single limit')
        ).length,
        rejected_cumulative: validationData.filter((r) =>
          r.contract_decision.includes('cumulative')
        ).length,
      },
      ...
      total_amount_attempted: validationData
  ...
  async displaySampleData() {
    ...
    await new Promise((resolve) => {
      this.db.all(
        'SELECT unique_id, first_name, last_name, wallet FROM campaign_prospects LIMIT 5',
        (err, rows) => {
          if (!err) {
            rows.forEach((row) => {});
          }
    ...
    await new Promise((resolve) => {
      this.db.all(
        'SELECT unique_id, first_name, last_name, contribution_amount FROM campaign_donors LIMIT 5',
        (err, rows) => {
          if (!err) {
            rows.forEach((row) => {});
          }
