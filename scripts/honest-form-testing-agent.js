...
class HonestFormTestingAgent {
  ...
  matchInputToData(inputDetails, data) {
    ...
    if (name.includes('address') || id.includes('address') || placeholder.includes('address'))
      return data.address;
    if (name.includes('city') || id.includes('city') || placeholder.includes('city'))
      return data.city;
    if (name.includes('state') || id.includes('state') || placeholder.includes('state'))
      return data.state;
    if (name.includes('zip') || name.includes('postal') || id.includes('zip')) return data.zip;
    if (name.includes('employer') || id.includes('employer') || placeholder.includes('employer'))
      return data.employer;
    if (
      name.includes('occupation') ||
      id.includes('occupation') ||
      placeholder.includes('occupation')
    )
    ...
    if (
      (name.includes('name') && !name.includes('last') && !name.includes('first')) ||
  ...
  async submitFormHonestly() {
    ...
    try {
      ...
      const submitButtons = await this.page.$$(
        'button[type="submit"], input[type="submit"], button:not([type]), .submit-btn, .donate-btn, [data-testid*="submit"]'
      );
  ...
  generateHonestReport() {
    ...
    const successfulSubmits = this.testResults.filter(
      (r) => r.result.submitResult?.submitted
    ).length;
    ...
    this.testResults.forEach((test, i) => {
      console.log(
        `${i + 1}. ${test.persona}: ${test.result.fieldsFilled || 0} fields filled, submitted: ${test.result.submitResult?.submitted || false}`
      );
    });
