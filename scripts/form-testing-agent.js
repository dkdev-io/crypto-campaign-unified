...
class FormTestingAgent {
  ...
  async fillFormFields(persona, testConfig) {
    ...
    await this.fillField('input[name="email"]', `${profile.first_name.toLowerCase()}.${profile.last_name.toLowerCase()}@test.com`);
  ...
  async verifyDatabaseEntry(persona, submissionResult) {
    ...
    try {
      ...
      const verification = {
        name: dbEntry.first_name === persona.profile.first_name && dbEntry.last_name === persona.profile.last_name,
