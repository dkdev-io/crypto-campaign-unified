...
class DualFormTestingAgent {
    ...
    try {
      ...
      for (let i = 0; i < inputs.length; i++) {
        try {
          ...
          if (fieldValue) {
            console.log(
              `📍 Filling ${inputInfo.name || inputInfo.id || inputInfo.type} with: ${fieldValue.substring(0, 20)}...`
            );
  ...
  generateTestReport() {
    ...
    console.log(
      `Homepage Form Success: ${homepagePasses}/${totalPersonas} (${((homepagePasses / totalPersonas) * 100).toFixed(1)}%)`
    );
    console.log(
      `Modal Form Success: ${modalPasses}/${totalPersonas} (${((modalPasses / totalPersonas) * 100).toFixed(1)}%)`
    );
    console.log(
      `Both Forms Success: ${bothFormsPasses}/${totalPersonas} (${((bothFormsPasses / totalPersonas) * 100).toFixed(1)}%)`
    );
