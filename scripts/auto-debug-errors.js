...
async function autoDebugErrors() {
  ...
  try {
    ...
    const homeResults = await page.evaluate(() => ({
      ...
      hasReactElements:
        document.querySelectorAll('[class*="bg-"], [class*="text-"], [class*="flex"]').length > 0,
    ...
    const authResults = await page.evaluate(() => ({
      ...
      hasAuthForm:
        document.querySelectorAll('form, input[type="email"], input[type="password"]').length > 0,
    ...
    console.log(
      `   React Loading: ${homeResults.reactLoaded || homeResults.rootHasContent ? 'PARTIAL' : 'FAILED'}`
    );
