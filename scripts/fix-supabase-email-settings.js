  ...
  try {
    ...
    const response = await fetch(`${supabaseUrl}/auth/v1/admin/settings`, {
      ...
      body: JSON.stringify({
        ...
        ADDITIONAL_REDIRECT_URLS: 'http://localhost:3000,http://localhost:5173,https://cryptocampaign.netlify.app'
      })
    })
...
async function testEmailAfterFix() {
  ...
  try {
    ...
    console.log(`🔗 Email will link to: ${process.env.VITE_APP_URL || 'https://cryptocampaign.netlify.app'}/auth?verified=true`)
