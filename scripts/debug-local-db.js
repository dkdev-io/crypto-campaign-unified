...
class DatabaseDebugger {
  ...
  async checkSampleData() {
    ...
    await new Promise((resolve) => {
      this.db.all(
        'SELECT unique_id, first_name, last_name FROM campaign_prospects LIMIT 3',
        (err, rows) => {
          if (err) {
    ...
    await new Promise((resolve) => {
      this.db.all(
        'SELECT unique_id, first_name, last_name, contribution_amount FROM campaign_donors LIMIT 3',
        (err, rows) => {
          if (err) {
