...
async function diagnoseLiveDeployment() {
  ...
  try {
    ...
    const pageInfo = await page.evaluate(() => ({
      ...
      hasViteScript: Array.from(document.getElementsByTagName('script')).some((s) =>
        s.src.includes('index')
      ),
      bodyClasses: document.body.className,
      metaTags: Array.from(document.getElementsByTagName('meta'))
        .map((m) => m.name || m.property)
        .filter(Boolean),
    }));
    ...
    const authPageInfo = await page.evaluate(() => ({
      hasReactElements: document.querySelectorAll(
        '[class*="bg-gradient"], [class*="crypto"], .container-responsive'
      ).length,
  ...
  diagnoseLiveDeployment()
    .then((results) => {
      if (results) {
        ...
        console.log(
          `   React Loading: ${results.pageInfo?.hasReactRoot && results.authPageInfo?.hasReactElements > 0 ? 'SUCCESS' : 'FAILED'}`
        );
        console.log(
          `   Breadcrumbs Present: ${results.authPageInfo?.breadcrumbElements > 0 ? 'YES' : 'NO'}`
        );
