...
class UltimateFormTestingAgent {
  ...
  async fillFormFields(profile, donation, inputFields) {
    ...
    const fieldMappings = [
      ...
      {
        match: ['amount', 'donation', 'contribution'],
      ...
      { match: ['phone'], value: profile.phone_number, type: 'phone' },
      {
        ...
        value: profile.wallet_address || '0x742d35Cc6639C0532fCb5FbF7b51f4FA8B4B8B34',
        type: 'wallet',
      },
    ];
  ...
  generateFinalReport() {
    ...
    if (successful.length > 0) {
      ...
      successful.forEach((result) => {
        ...
        console.log(
          `  ✅ ${result.persona}: ${fillSuccess}/${fillAttempted} fields filled, submitted successfully`
        );
      });
    }
    ...
    if (failed.length > 0) {
      failed.forEach((result) => {
        console.log(
          `  ❌ ${result.persona}: ${result.error || result.steps.submission?.error || 'Unknown error'}`
        );
      });
    }
