  ...
  async analyzeCurrentPage() {
    ...
    try {
      const analysis = await this.page.evaluate(() => {
        ...
        const hasFormKeywords = ['amount', 'email', 'name', 'address', 'donation', 'contribute', 'payment'].some(keyword => 
    ...
    try {
      ...
      for (const input of inputs) {
        try {
          ...
          if (fieldValue) {
            console.log(`📍 Filling ${inputInfo.name || inputInfo.id || inputInfo.type} with: ${fieldValue}`);
