...
async function testBreadcrumbPresence() {
  ...
  try {
    ...
    const navElements = await page.evaluate(() => {
      ...
      document
        .querySelectorAll('nav[aria-label*="readcrumb"], nav[aria-label*="Breadcrumb"]')
        .forEach((nav) => {
      ...
      document.querySelectorAll('*').forEach((el) => {
        if (
          el.textContent &&
          el.textContent.toLowerCase().includes('campaign') &&
    ...
    const hasBreadcrumbComponent =
      pageContent.includes('CampaignBreadcrumb') ||
    ...
    const hiddenBreadcrumbs = await page.evaluate(() => {
      ...
      allDivs.forEach((div) => {
        ...
        if (
          (div.textContent.includes('Home') || div.textContent.includes('Campaign')) &&
    ...
    hiddenBreadcrumbs.forEach((el, i) => {
      console.log(
        `     ${i + 1}. "${el.textContent}" (display: ${el.display}, visibility: ${el.visibility})`
      );
    });
  ...
  testBreadcrumbPresence()
    .then((results) => {
      if (results) {
        console.log('\n📋 SUMMARY:');
        console.log(
          `   Breadcrumbs working: ${results.breadcrumbNavsFound || results.anyNavsFound}`
        );
