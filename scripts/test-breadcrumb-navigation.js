...
async function testBreadcrumbNavigation() {
  ...
  try {
    ...
    const homeBreadcrumbs = await page.$eval('nav[aria-label="Breadcrumb"]', el => el ? el.textContent : null).catch(() => null);
    results.homepage.tested = true;
    results.homepage.hasBreadcrumbs = !!homeBreadcrumbs;
    results.homepage.details = homeBreadcrumbs ? `Found: "${homeBreadcrumbs.trim()}"` : 'No breadcrumbs (expected for homepage)';
    
    console.log(`   Homepage breadcrumbs: ${results.homepage.hasBreadcrumbs ? '✅ Found' : '❌ Not found (expected)'}`);
    ...
    console.log(`   ${results.setupPage.protected ? '✅' : '⚠️ '} Route protection: ${results.setupPage.protected ? 'Working (redirected to auth)' : 'Not redirected'}`);
    ...
    const homeNavigation = await page.evaluate(() => {
      const homeLink = document.querySelector('nav[aria-label="Breadcrumb"] a[href="/"], nav[aria-label="Breadcrumb"] button');
      if (homeLink) {
        homeLink.click();
        return true;
      }
    ...
    if (homeNavigation) {
      ...
      console.log(`   ${navigatedHome ? '✅' : '❌'} Home navigation: ${navigatedHome ? 'Working' : 'Failed'}`);
    }
    ...
    console.log(`   Breadcrumbs: ${results.homepage.hasBreadcrumbs ? 'Found' : 'Not found (expected)'}`);
    ...
    console.log(`   Protection: ${results.setupPage.protected ? '✅ Working' : '⚠️  Not protected'}`);
    ...
    console.log(`   Live Site Status: ${authWorking ? '✅ DEPLOYED SUCCESSFULLY' : '❌ DEPLOYMENT ISSUE'}`);
