...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImttZXBjZHNrbG5ueG9rb2ltdnpvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzU1NDYyNDgsImV4cCI6MjA1MTEyMjI0OH0.nJHF3rG3N6kGyzDD_vq2D4t9gLCLBNDVsLqHs9EbKfE';
const SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImttZXBjZHNrbG5ueG9rb2ltdnpvIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTczNTU0NjI0OCwiZXhwIjoyMDUxMTIyMjQ4fQ.HgFnPg4pLT3pqX-tVlB3HhNWdLgOGf0J3X8-mTgCmPo';
...
async function executeSQLViaAPI() {
  ...
  try {
    ...
    if (createFunctionResponse.ok) {
      ...
      const executeFunctionResponse = await fetch(`${SUPABASE_URL}/rest/v1/rpc/fix_campaigns_table`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${SERVICE_KEY}`,
          'Content-Type': 'application/json',
    ...
    const tableCheckResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/campaigns?select=*&limit=1`,
      {
        headers: {
          'Authorization': `Bearer ${SERVICE_KEY}`,
          'apikey': SERVICE_KEY
