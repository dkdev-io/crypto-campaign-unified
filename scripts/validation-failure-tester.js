...
class ValidationFailureTester {
  ...
  async fillFormForFailureCase(page, prospect, failureCase) {
    ...
    // Fill form fields
    const fieldMappings = [
      { selectors: ['input[name*="address"]', 'input[placeholder*="address"]'], value: prospect.address_line_1 },
      ...
      { selectors: ['input[name*="wallet"]', 'input[placeholder*="wallet"]'], value: '0x742d35Cc6639C0532fCb5FbF7b51f4FA8B4B8B34' }
    ];
  ...
  async submitAndCheckValidation(page, failureCase) {
    ...
    try {
      ...
      await page.screenshot({ path: `test-results/validation-test-${failureCase.unique_id}-before.png` });
      ...
      await page.screenshot({ path: `test-results/validation-test-${failureCase.unique_id}-after.png` });
      ...
      const validationCheck = await page.evaluate(() => {
        ...
        const errorSelectors = [
          ...
          '.validation-error', '[class*="invalid"]',
          '.form-error', '[aria-invalid="true"]'
        ];
