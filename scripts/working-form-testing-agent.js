...
class WorkingFormTestingAgent {
      ...
      if (donateButtons.length > 0) {
        try {
          await this.page.evaluate(() => {
            const donateEl = Array.from(document.querySelectorAll('*')).find(el => 
              el.textContent && 
              el.textContent.trim().toLowerCase().includes('donate') &&
              (el.tagName === 'BUTTON' || el.tagName === 'A')
            );
  ...
  async fillVisibleForm(persona, context) {
    ...
    try {
      ...
      const inputs = await this.page.$$('input:not([type="hidden"]):not([type="submit"]):not([type="button"]), select, textarea');
      ...
      for (let i = 0; i < inputs.length; i++) {
        try {
          ...
          if (value) {
            console.log(`📍 Filling ${inputInfo.name || inputInfo.id || inputInfo.placeholder} with: ${value}`);
  ...
  smartMatch(inputInfo, formData) {
    ...
    if (name.includes('first') || id.includes('first') || placeholder.includes('first')) return formData.first;
    if (name.includes('last') || id.includes('last') || placeholder.includes('last')) return formData.last;
    if (type === 'email' || name.includes('email') || id.includes('email')) return formData.email;
    if (type === 'tel' || name.includes('phone') || placeholder.includes('phone')) return formData.phone;
    ...
    if (name.includes('amount') || placeholder.includes('amount') || placeholder.includes('$')) return formData.amount;
    ...
    try {
      ...
      if (submitted) {
        ...
        return {
          ...
          submitted: true,
          message: result.hasSuccess ? 'Form submitted successfully' : 
                   result.hasError ? 'Form submission failed' : 'Form submitted - status unclear',
          result
        };
      } else {
        return {
