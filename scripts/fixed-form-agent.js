...
class FixedFormAgent {
  ...
  smartMatch(field, data) {
    ...
    if (allText.includes('first') && (allText.includes('name') || allText.includes('nome')))
      return data.firstName;
    if (allText.includes('last') && (allText.includes('name') || allText.includes('nome')))
      return data.lastName;
    if (allText.includes('email') || allText.includes('e-mail')) return data.email;
    if (allText.includes('phone') || allText.includes('tel') || field.type === 'tel')
      return data.phone;
    if (allText.includes('address') && !allText.includes('wallet') && !allText.includes('crypto'))
      return data.address;
    ...
    if (allText.includes('zip') || allText.includes('postal') || allText.includes('cep'))
      return data.zip;
    if (allText.includes('employer') || allText.includes('company')) return data.employer;
    if (allText.includes('occupation') || allText.includes('job') || allText.includes('profess'))
      return data.occupation;
    if (
      allText.includes('amount') ||
      allText.includes('contribution') ||
  ...
  async testPersonaOnBothForms(persona, testIndex, totalTests) {
      ...
      if (!donateButton) {
        ...
        await this.page.evaluate(() => {
          ...
          const donateBtn = buttons.find((btn) =>
            btn.textContent.trim().toLowerCase().includes('donate')
          );
    ...
    const homeSuccess =
      results.homepageTest?.success && results.homepageTest?.submitResult?.submitted;
    ...
    console.log(
      `   Homepage: ${results.homepageTest?.fieldsFilled || 0} fields, submitted: ${results.homepageTest?.submitResult?.submitted || false}`
    );
    console.log(
      `   Modal: ${results.modalTest?.fieldsFilled || 0} fields, submitted: ${results.modalTest?.submitResult?.submitted || false}`
    );
  ...
  generateFixedReport() {
    ...
    this.testResults.forEach((result) => {
      ...
      totalFieldsFilled +=
        (result.homepageTest?.fieldsFilled || 0) + (result.modalTest?.fieldsFilled || 0);
    ...
    this.testResults.forEach((result, i) => {
      ...
      console.log(
        `   Homepage: ${result.homepageTest?.fieldsFilled || 0} fields, submitted: ${result.homepageTest?.submitResult?.submitted || false}`
      );
      console.log(
        `   Modal: ${result.modalTest?.fieldsFilled || 0} fields, submitted: ${result.modalTest?.submitResult?.submitted || false}`
      );
    });
