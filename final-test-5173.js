...
(async () => {
  ...
  console.log('📄 Page shows:', pageContent.includes('Step 2') ? 'Step 2 - Committee' : 'Step 1 or other');
  ...
  if (committeeInput) {
    ...
    const submitButton = await page.evaluateHandle(() => {
      ...
      return buttons.find(btn => btn.textContent.includes('Save Committee') && btn.textContent.includes('Continue'));
    });
    ...
    if (submitButton.asElement()) {
      ...
      if (finalContent.includes('Committee Information Saved') || finalContent.includes('Step 3') || finalContent.includes('Bank')) {
