...
async function verifyFinalFixes() {
  ...
  try {
    ...
    for (const route of testRoutes) {
      ...
      try {
        ...
        const analysis = await page.evaluate((homeRef) => {
          ...
          const elementsWithInlineColors = Array.from(
        ...
        console.log(
          `  Emojis: ${analysis.emojiCount === 0 ? '✅ None' : '❌ Found ' + analysis.emojiCount + ': ' + analysis.emojisFound.join(', ')}`
        );
        console.log(`  Background: ${analysis.backgroundColor}`);
        console.log(
          `  Hardcoded colors: ${analysis.hardcodedColors.length === 0 ? '✅ None' : '⚠️ Found ' + analysis.hardcodedColors.length}`
        );
        console.log(
          `  Fonts using design system: ${analysis.fontAnalysis.filter((f) => f.usesDesignSystem).length}/${analysis.fontAnalysis.length}`
        );

        if (analysis.hardcodedColors.length > 0) {
          console.log(
            `    Examples: ${analysis.hardcodedColors
              .slice(0, 2)
              .map((el) => el.style)
              .join('; ')}`
          );
    ...
    const emojisRemovedPercent = Math.round(
      (validResults.filter((r) => r.emojiCount === 0).length / validResults.length) * 100
    );
    const hardcodedColorsRemovedPercent = Math.round(
      (validResults.filter((r) => r.hardcodedColors.length === 0).length / validResults.length) *
        100
    );
    ...
    const designSystemHeadings = validResults.reduce(
      (sum, r) => sum + (r.fontAnalysis?.filter((f) => f.usesDesignSystem).length || 0),
      0
    );
    const fontPercent =
      totalHeadings > 0 ? Math.round((designSystemHeadings / totalHeadings) * 100) : 0;

    console.log(`Font sizes using design system: ${fontPercent}%`);

    const overallScore = Math.round(
      (emojisRemovedPercent + hardcodedColorsRemovedPercent + fontPercent) / 3
    );
