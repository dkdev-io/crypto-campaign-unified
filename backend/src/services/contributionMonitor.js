...
class ContributionMonitor {
  ...
  async checkRapidContributions() {
    try {
      ...
      const { data: rapidContributors } = await this.supabase.rpc('get_rapid_contributors', {
  ...
  async checkComplianceViolations() {
    try {
      ...
      const { data: violations } = await this.supabase.rpc('check_fec_violations', {
        limit_amount: 3300,
      });
  ...
  async checkDatabaseHealth() {
    try {
      const { error } = await this.supabase.from('contributions').select('id').limit(1);
      return !error;
    } catch {
      return false;
    }
  ...
  async getDashboardData() {
    const [totalContributions, totalRejections, recentAlerts, systemHealth] = await Promise.all([
  ...
  async getTotalContributions() {
    const { count } = await this.supabase.from('contributions').select('id', { count: 'exact' });
    return count || 0;
  }
