...
class RetryHandler {
    ...
    shouldRetryNow(rejection) {
        ...
        const lastRetry = rejection.last_retry_at ? new Date(rejection.last_retry_at) : new Date(rejection.created_at);
        const timeSinceLastRetry = Date.now() - lastRetry.getTime();
        
        // Get appropriate interval based on retry count
        const requiredInterval = this.retryIntervals[Math.min(retryCount - 1, this.retryIntervals.length - 1)];
