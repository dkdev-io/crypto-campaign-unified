...
router.get('/statistics', async (req, res) => {
    try {
        ...
        // Execute queries
        const [successResult, rejectedResult] = await Promise.all([
            successQuery,
            rejectedQuery
        ]);
        ...
        const successfulAmount = successResult.data?.reduce(
            (sum, c) => sum + parseFloat(c.amount_usd || 0),
            0
        ) || 0;

        const rejectedAmount = rejectedResult.data?.reduce(
