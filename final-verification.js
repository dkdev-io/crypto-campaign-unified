  ...
  try {
    ...
    const currentState = await page.evaluate(() => {
      ...
      return {
        ...
        setupContainerBg: setupContainer ? window.getComputedStyle(setupContainer).backgroundColor : 'none',
    ...
    try {
      const campaignInput = await page.waitForSelector('input[name="campaignName"], [placeholder*="campaign"]', { timeout: 3000 });
      await campaignInput.type('Test Campaign');
      
      const websiteInput = await page.waitForSelector('input[name="website"], [type="url"]', { timeout: 3000 });
      await websiteInput.type('https://example.com');
      
      const nextBtn = await page.waitForSelector('button:has-text("Next"), .btn-primary', { timeout: 3000 });
      await nextBtn.click();
