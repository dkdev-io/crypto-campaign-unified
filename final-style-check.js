...
async function checkStyling() {
  ...
  for (const pageInfo of pagesToTest) {
    ...
    try {
      ...
      try {
        const bypassButton = await page.waitForSelector(
          '[class*="yellow"]:has-text("BYPASS"), button[class*="yellow"]',
          { timeout: 2000 }
        );
        if (bypassButton) {
      ...
      await page.screenshot({
        path: `/private/tmp/final-${pageInfo.name.toLowerCase().replace(' ', '-')}.png`,
      });
      ...
      const styleCheck = await page.evaluate(() => {
        ...
        const mainContainer = document.querySelector(
          '[style*="crypto-navy"], .min-h-screen, [style*="214 100% 21%"]'
        );
        ...
        const isDarkBlue =
          bgColor.includes('rgb(26, 35, 126)') ||
      ...
      const isConsistent =
        styleCheck.darkBlueBackground &&
        styleCheck.whiteInputs &&
        styleCheck.interFont &&
        styleCheck.noEmojis;
      ...
      if (isConsistent) {
        ...
        console.log(
          `    - Dark blue background: ${styleCheck.darkBlueBackground ? '✅' : '❌'} (${styleCheck.bgColor})`
        );
        console.log(
          `    - White inputs: ${styleCheck.whiteInputs ? '✅' : '❌'} (${styleCheck.whiteInputCount}/${styleCheck.inputCount})`
        );
        console.log(
          `    - Inter font: ${styleCheck.interFont ? '✅' : '❌'} (${styleCheck.fontFamily})`
        );
        console.log(
          `    - No emojis: ${styleCheck.noEmojis ? '✅' : '❌'} (${styleCheck.emojiCount} found)`
        );
      }
    } catch (error) {
