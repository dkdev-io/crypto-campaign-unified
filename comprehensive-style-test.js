...
async function comprehensiveStyleTest() {
  ...
  for (const pageInfo of allPages) {
    ...
    try {
      ...
      // Take screenshot
      await page.screenshot({ path: `/private/tmp/comprehensive-${pageInfo.name.toLowerCase().replace(/[ \/]/g, '-')}.png` });
      
      // Check if page has proper styling
      const styleCheck = await page.evaluate(() => {
        // Check for dark blue background
        const darkElement = document.querySelector('[style*="crypto-navy"], .min-h-screen[style*="214"], [style*="1a237e"]');
        const hasDarkBackground = !!darkElement || 
        ...
        const whiteInputs = Array.from(inputs).filter(input => {
          ...
          return style.backgroundColor === 'rgb(255, 255, 255)' || style.backgroundColor.includes('255, 255, 255');
        });
        
        // Check Inter font
        const hasInterFont = getComputedStyle(document.body).fontFamily.toLowerCase().includes('inter');
      ...
      if (styleCheck.totalInputCount === 0 || styleCheck.whiteInputCount === styleCheck.totalInputCount) score += 25;
      ...
      console.log(`    📝 White inputs: ${styleCheck.whiteInputCount}/${styleCheck.totalInputCount} ${styleCheck.totalInputCount === 0 || styleCheck.whiteInputCount === styleCheck.totalInputCount ? '✅' : '❌'}`);
      console.log(`    🔤 Inter font: ${styleCheck.hasInterFont ? '✅' : '❌'}`);
      console.log(`    😀 No emojis: ${styleCheck.emojiCount === 0 ? '✅' : '❌'} (${styleCheck.emojiCount} found)`);
