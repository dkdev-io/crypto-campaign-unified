...
async function navigateToStep2() {
  ...
  try {
    ...
    try {
      // Fill campaign name
      const campaignNameInput = await page.waitForSelector('input[name="campaignName"], input[placeholder*="campaign"], input[placeholder*="Campaign"]', { timeout: 5000 });
      await campaignNameInput.type('Test Campaign');
      
      // Fill website
      const websiteInput = await page.waitForSelector('input[name="website"], input[type="url"], input[placeholder*="website"]', { timeout: 5000 });
      await websiteInput.type('https://example.com');
      
      // Click Next button
      const nextButton = await page.waitForSelector('button:has-text("Next"), button:has-text("Continue"), .btn-primary', { timeout: 5000 });
    ...
    const step2Info = await page.evaluate(() => {
      const stepText = document.querySelector('[class*="step"], .step-indicator')?.textContent || '';
