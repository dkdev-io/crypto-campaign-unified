...
async function main() {
  ...
  const campaignTreasury = process.env.CAMPAIGN_TREASURY || "0x1234567890123456789012345678901234567890";
  ...
  // Estimate gas
  const deploymentData = CampaignContributions.interface.encodeDeploy([campaignTreasury, initialOwner]);
  ...
  const abiData = {
    ...
    abi: CampaignContributions.interface.fragments.map(fragment => fragment.format("json")).map(JSON.parse)
  };
  ...
  if (network.chainId === 1n || network.chainId === 11155111n || network.chainId === 8453n || network.chainId === 84532n) {
    console.log("\\n📝 Next steps:");
    console.log("1. Verify contract on block explorer:");
    console.log(`   npx hardhat verify --network ${network.name} ${contractAddress} "${campaignTreasury}" "${initialOwner}"`);
