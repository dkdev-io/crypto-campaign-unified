...
describe("CampaignContributions", function () {
  ...
  describe("KYC Management", function () {
    ...
    it("Should emit KYCStatusUpdated event when verifying", async function () {
      await expect(
        campaignContract.connect(kycVerifier).verifyKYC(contributor1.address)
  ...
  describe("Contributions", function () {
    ...
    it("Should allow multiple contributions within limits", async function () {
      ...
      const [, , , contributorInfo] = await campaignContract.getContributorInfo(contributor1.address);
  ...
  describe("Admin Functions", function () {
    it("Should allow owner to update ETH price", async function () {
      ...
      await expect(
        campaignContract.setEthPrice(newPrice)
    ...
    it("Should allow owner to update treasury address", async function () {
      ...
      await expect(
        campaignContract.setCampaignTreasury(newTreasury.address)
  ...
  describe("Edge Cases", function () {
    ...
    it("Should handle receive() function for direct transfers", async function () {
      ...
      await expect(
        contributor1.sendTransaction({ to: await campaignContract.getAddress(), value: contributionAmount })
      ).to.emit(campaignContract, "ContributionAccepted");
    });
    ...
    it("Should reject direct transfers for non-KYC users", async function () {
      ...
      await expect(
        contributor2.sendTransaction({ to: await campaignContract.getAddress(), value: contributionAmount })
      ).to.be.revertedWith("CampaignContributions: direct transfer rejected - use contribute() function");
    });
  });
});
