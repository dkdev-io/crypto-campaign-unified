...
const EnhancedDonorForm = ({ campaignId }) => {
  ...
  const validateForm = () => {
    ...
    if (
      !formData.citizenshipConfirmed ||
      !formData.ownFundsConfirmed ||
      !formData.ageConfirmed ||
      !formData.notContractorConfirmed ||
      !formData.personalCardConfirmed ||
  ...
  const saveContribution = async (
    ...
    try {
      const contributionData = {
        ...
        recurring_frequency:
          formData.donationType === 'recurring' ? formData.recurringFrequency : null,
        recurring_start_date:
          formData.donationType === 'recurring' ? formData.recurringStartDate : null,
        recurring_end_date:
          formData.donationType === 'recurring' && formData.recurringEndDate
            ? formData.recurringEndDate
            : null,
        recurring_amount:
          formData.donationType === 'recurring' ? parseFloat(formData.amount) : null,
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        {campaignId && (
          <p style={{ fontSize: '0.9rem', color: '#666' }}>Campaign ID: {campaignId}</p>
        )}
      </div>
    );
  }
  ...
  if (errorMessage && !campaignData) {
    return (
      <div
        style={{ padding: '2rem', textAlign: 'center', background: '#f8d7da', borderRadius: '8px' }}
      >
        <h3>Unable to Load Campaign</h3>
        <p>{errorMessage}</p>
        <p style={{ fontSize: '0.9rem', color: '#666' }}>
          Campaign ID: {campaignId || 'None provided'}
        </p>
        <button
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <div
          style={{ marginTop: '1rem', background: 'white', padding: '1.5rem', borderRadius: '8px' }}
        >
            ...
            >
              ...
              {contributionProjection.autoCancelDate && (
                <p style={{ color: '#dc3545', fontWeight: 'bold' }}>
                  ...
                  {new Date(contributionProjection.autoCancelDate).toLocaleDateString()}
                  <br />
  ...
  return (
    <div className="donor-form container-responsive crypto-card" style={{ maxWidth: '600px' }}>
      ...
      {!walletInfo?.isConnected && formData.amount && (
        ...
        >
          ⚠️ <strong>Wallet Connection Required for Validation</strong>
          <br />
          To ensure FEC compliance and validate contribution limits, please connect your wallet
          using the Web3 component above. This is required for proper validation of both traditional
          and crypto payments.
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
              ...
              {contributionProjection && (
                ...
                >
                  <p style={{ margin: 0, fontSize: '0.9rem' }}>
                    <strong>Projection:</strong> {contributionProjection.paymentCount} payments
                    totaling ${contributionProjection.totalAmount.toFixed(2)}
                  </p>
                  {contributionProjection.willExceedLimit && (
                    <p style={{ margin: '0.5rem 0 0 0', color: '#dc3545', fontSize: '0.9rem' }}>
                      ⚠️ Will auto-cancel on{' '}
                      {new Date(contributionProjection.autoCancelDate).toLocaleDateString()} to
                      comply with FEC $3,300 limit
                    </p>
                  )}
                </div>
              )}
          ...
          {limitCheck && formData.amount && (
            ...
            >
              {limitCheck.canContribute ? (
                ...
                <span style={{ color: '#721c24' }}>✗ {limitCheck.message}</span>
              )}
            </div>
          )}
        </div>
        ...
        >
          ...
          >
            ...
            />
            <span>
              I certify that I am a U.S. citizen or lawfully admitted permanent resident (i.e.,
              green card holder).
            </span>
          </label>
          ...
          >
            <input
              ...
              style={{ marginTop: '0.25rem' }}
            />
            <span>I am at least eighteen years old.</span>
          </label>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, notContractorConfirmed: e.target.checked })
              }
              style={{ marginTop: '0.25rem' }}
            />
          ...
          >
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, personalCardConfirmed: e.target.checked })
              }
              style={{ marginTop: '0.25rem' }}
            />
            <span>
        ...
        >
          ...
          >
            ...
            <span>
              ...
              marketing messages from the campaign. Message & data rates may apply. Reply HELP for
              help, STOP to end.
            </span>
          </label>
          ...
          >
            <input
              ...
              style={{ marginTop: '0.25rem' }}
            />
            <span>I am not an executive of a fossil fuel company.</span>
          </label>
          ...
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, bluetokensTermsAccepted: e.target.checked })
              }
              style={{ marginTop: '0.25rem' }}
            />
            <span>
              I understand the purchase of BluTokens will be shipped to me separately with no cash
              value.
            </span>
          </label>
        </div>
        ...
        {paymentMethod === 'traditional' && (
          <button
            ...
            style={{
              ...
              cursor:
                isSubmitting || (limitCheck && !limitCheck.canContribute)
                  ? 'not-allowed'
                  : 'pointer',
            }}
          >
        ...
        {paymentMethod === 'crypto' && walletInfo?.isConnected && (
          <button
            ...
            style={{
              ...
              cursor:
                isProcessingCrypto || (limitCheck && !limitCheck.canContribute)
                  ? 'not-allowed'
                  : 'pointer',
            }}
          >
