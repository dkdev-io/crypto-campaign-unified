...
const AdminDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [campaignsResponse, transactionsResponse, contributionsResponse] =
      ...
      const campaigns =
        campaignsResponse.status === 'fulfilled' ? campaignsResponse.value.data || [] : [];
      const transactions =
        transactionsResponse.status === 'fulfilled' ? transactionsResponse.value.data || [] : [];
      const contributions =
        contributionsResponse.status === 'fulfilled' ? contributionsResponse.value.data || [] : [];
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-32 bg-secondary rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${(item.count / Math.max(...stats.userGrowth.map((g) => g.count))) * 100}%`,
                      }}
                    ></div>
                  </div>
