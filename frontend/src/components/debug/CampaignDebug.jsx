    ...
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {campaigns.length === 0 ? (
        <div style={{ background: '#fff3cd', padding: '1rem', borderRadius: '4px' }}>
          ...
          <p>
            You need to create a campaign first using the setup wizard at <code>/</code>
          </p>
        </div>
      ) : (
        <div>
          ...
          {campaigns.map((campaign, index) => (
            ...
            >
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 3fr', gap: '1rem' }}>
                ...
                <div>
                  <div>
                    <strong>Name:</strong> {campaign.campaign_name}
                  </div>
                  <div>
                    <strong>Email:</strong> {campaign.email}
                  </div>
                  <div>
                    <strong>Candidate:</strong> {campaign.candidate_name || 'Not set'}
                  </div>
                  <div>
                    <strong>Website:</strong> {campaign.website}
                  </div>
                  <div>
                    <strong>Suggested Amounts:</strong> {JSON.stringify(campaign.suggested_amounts)}
                  </div>
                  <div>
                    <strong>Theme:</strong>{' '}
                    <span style={{ color: campaign.theme_color }}>{campaign.theme_color}</span>
                  </div>
                </div>
              </div>
              <div
      ...
      <div
        style={{ marginTop: '2rem', padding: '1rem', background: '#e7f3ff', borderRadius: '4px' }}
      >
        <h4>Debug Actions</h4>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <a href="/supabase-check" style={{ color: '#007bff' }}>
            Supabase Check
          </a>
          <a href="/test-form" style={{ color: '#007bff' }}>
            Test Form (Fake ID)
          </a>
        </div>
      </div>
