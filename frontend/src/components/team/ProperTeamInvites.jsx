  ...
  const sendInvites = async (e) => {
    ...
    try {
      ...
      const sentCount = inviteResults.filter(
        (r) => r.status === 'sent' || r.status === 'magic-link'
      ).length;
      ...
      if (sentCount > 0 && manualCount === 0) {
        ...
        alert(
          `⚠️ Partial success: ${sentCount} email(s) sent, ${manualCount} need manual sharing.`
        );
      } else {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <form onSubmit={sendInvites}>
        ...
        >
          ⚠️ <strong>Important:</strong> Use real email addresses. Test emails like
          "test@example.com" will be rejected.
        </div>
