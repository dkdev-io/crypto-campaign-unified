...
const RealWorkingInvites = ({ campaignId }) => {
  ...
  const validateEmail = (email) => {
    ...
    if (blockedDomains.includes(domain)) {
      return { valid: false, reason: `${domain} emails are not allowed. Use a real email address.` }
    }
    ...
    try {
      ...
      for (const invite of validInvites) {
        try {
          ...
          const inviteToken = crypto.randomUUID ? crypto.randomUUID() : `invite-${Date.now()}-${Math.random()}`
      ...
      if (sentCount > 0 && errorCount === 0) {
        alert(`✅ Success! ${sentCount} invitation email(s) sent.${existsCount > 0 ? `\n${existsCount} user(s) already have accounts.` : ''}`)
    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ 
        ...
        <p style={{ margin: '0.5rem 0 0 0', fontSize: '14px' }}>
          Invited users will receive verification emails to set up their accounts.
          Please use real, accessible email addresses (not test@example.com).
        </p>
      </div>
        ...
        <div style={{
          ...
          {results.map((result, index) => (
            <div key={index} style={{
              ...
              borderRadius: '4px',
              border: `2px solid ${
                result.status === 'sent' ? '#28a745' : 
                result.status === 'exists' ? '#17a2b8' : 
                '#dc3545'
              }`
            }}>
