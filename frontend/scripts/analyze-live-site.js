  ...
  try {
    ...
    const pagesToCheck = [
      ...
      },
      {
        ...
        expectedChanges: ['Navy background', 'Navigation styling updates'],
      },
      {
        name: 'Setup Page',
    ...
    for (const pageInfo of pagesToCheck) {
      ...
      try {
        ...
        console.log(
          `   Navy elements: ${navyElements.blueCount}, White elements: ${navyElements.whiteCount}`
        );
      } catch (error) {
    ...
    results.forEach((result) => {
      ...
      const hasNavyBackground =
        result.bodyBackground.includes('rgb(26, 35, 126)') ||
      ...
      const pagePercentage = Math.round(
        (pageImplementedCount / result.expectedChanges.length) * 100
      );
    ...
    const resultsPath =
      '/Users/Danallovertheplace/crypto-campaign-unified/frontend/scripts/site-analysis-results.json';
    fs.writeFileSync(
      resultsPath,
      JSON.stringify(
        {
