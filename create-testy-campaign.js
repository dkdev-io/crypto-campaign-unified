...
async function createTestyCampaign() {
  ...
  try {
    ...
    for (const input of urlInputs) {
      ...
      if (
        type === 'url' ||
        placeholder.toLowerCase().includes('site') ||
    ...
    let embedCode = '';
    const codeElements = await page.$$(
      'textarea, pre, code, div[style*="monospace"], div[style*="Monaco"]'
    );
    ...
    for (const element of allElements) {
      try {
        ...
        if (
          text.includes('localhost:5174/testy') ||
          (text.includes('/testy') && text.includes('http'))
        ) {
