...
async function comprehensiveUITest() {
  ...
  try {
    ...
    for (const pageInfo of donorPages) {
      try {
        ...
        if (pageInfo.useBypass) {
          ...
          try {
            const bypassButton = await page.waitForSelector('button:has-text("DEV BYPASS")', {
              timeout: 3000,
            });
        ...
        const pageTest = await page.evaluate((homeRef) => {
          ...
          return {
            ...
            headingUsesDesignSystem: mainHeading
              ?.getAttribute('style')
              ?.includes('var(--text-heading'),
          };
        }, homeReference);
        ...
        console.log(
          `    Emojis: ${pageTest.emojiCount === 0 ? '✅ None' : '❌ ' + pageTest.emojiCount + ' found'}`
        );
        console.log(
          `    Background: ${pageTest.backgroundMatches ? '✅ Matches home' : '❌ Different'}`
        );
    ...
    const allPages = [...results.donorWorkflow, ...results.campaignWorkflow].filter(
      (p) => !p.error
    );
    ...
    const overallScore = Math.round(
      (results.summary.emojisRemoved +
        results.summary.backgroundsConsistent +
        results.summary.fontsConsistent) /
        3
    );
