...
async function testUIConsistency() {
      ...
      try {
        ...
        const hardcodedColors = await page.evaluate(() => {
          const elements = document.querySelectorAll(
            '*[style*="color"], *[class*="text-"], *[class*="bg-"]'
          );
          const issues = [];
  ...
  results.pages.forEach((page) => {
    console.log(`\n${page.name} (${page.url}):`);
    console.log(
      `  Emojis: ${page.emojis.length === 0 ? '✅ None' : '❌ ' + page.emojis.length + ' found'}`
    );
    console.log(
      `  Color Issues: ${page.hardcodedColors.length === 0 ? '✅ None' : '⚠️ ' + page.hardcodedColors.length + ' found'}`
    );
