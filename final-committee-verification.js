...
async function finalVerification() {
  ...
  try {
    ...
    const devBypassFound = buttons.some(
      (text) => text.includes('DEV BYPASS') || text.includes('Setup')
    );
    if (devBypassFound) {
    ...
    const allInputs = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('input')).map(
        (input) => input.placeholder || input.type
      );
    });
    ...
    if (committeeNameInput) {
      ...
      if (submitButton) {
        ...
        await page.evaluate(() => {
          ...
          for (let btn of buttons) {
            if (
              btn.textContent.includes('Save Committee') ||
