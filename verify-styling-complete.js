...
async function verifyStyleConsistency() {
  ...
  for (const pageInfo of pages) {
    ...
    try {
      ...
      await page.screenshot({
        path: `/private/tmp/verification-${pageInfo.name.toLowerCase().replace(' ', '-')}.png`,
      });

      // Check for auth bypass and use it if needed
      if (
        pageInfo.url.includes('/auth') ||
        pageInfo.url.includes('YourInfo') ||
        pageInfo.url.includes('Committee') ||
        pageInfo.url.includes('Bank') ||
        pageInfo.url.includes('Website')
      ) {
        try {
          const bypassButton = await page.$(
            'button:has-text("DEV BYPASS"), button:has-text("BYPASS")'
          );
          if (bypassButton) {
            ...
            await page.screenshot({
              path: `/private/tmp/verification-${pageInfo.name.toLowerCase().replace(' ', '-')}-bypassed.png`,
            });
          }
        } catch (e) {
      ...
      const comparison = {
        ...
        inputsHaveWhiteBackground: pageStyles.inputColors.every(
          (input) =>
      ...
      console.log(
        `  😀 No Emojis: ${comparison.emojisFound === 0 ? '✅' : '❌'} (${comparison.emojisFound} found)`
      );
  ...
  const averageScore =
    results.reduce((sum, result) => sum + (result.consistencyScore || 0), 0) / totalPages;
  ...
  console.log(
    `Perfect pages: ${perfectPages}/${totalPages} (${((perfectPages / totalPages) * 100).toFixed(1)}%)`
  );
  console.log(`\nDetailed Results:`);
